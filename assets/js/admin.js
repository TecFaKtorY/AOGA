/* ═══════════════════════════════════════
   Admin Panel v3 — HOLEPIC Manager Dashboard
   ═══════════════════════════════════════ */
...
function getDefaultData() {
  return {
    ...
    messages: [], books: [], verses: [], sections: [], events: [],
    uploads: [], contact: {}, seo: {}
  };
}
...
function renderSectionsList() {
  ...
  count.textContent = items.length;
  if (!items.length) {
    list.innerHTML = '<p style="color:#64748B;font-size:0.82rem;">No custom sections yet.</p>';
    return;
  }
  const placementLabels = {
    afterHero: 'After Hero', afterAbout: 'After About', afterMessages: 'After Messages',
    afterBooks: 'After Books', afterVerses: 'After Verses', afterEvents: 'After Events',
  ...
  items.forEach((s, i) => {
    ...
    div.innerHTML = `
      <div class="item-info">
        ...
        <div class="item-sub">${placementLabels[s.placement] || 'After Events'} ${s.image ? '· 🖼️ image' : ''} ${s.buttonText ? '· 🔘 button' : ''}</div>
      </div>
      <button class="btn-remove" data-index="${i}">&times;</button>`;
    div.querySelector('.btn-remove').addEventListener('click', () => {
...
$('addSectionBtn')?.addEventListener('click', () => {
  ...
  const maxId = data.sections.reduce((max, s) => Math.max(max, s.id || 0), 0);
  data.sections.push({
    id: maxId + 1,
    title,
    body: $('newSectionBody').value.trim(),
    image: $('newSectionImage').value.trim(),
    buttonText: $('newSectionBtnText').value.trim(),
    buttonLink: $('newSectionBtnLink').value.trim(),
    placement: $('newSectionPlacement').value
  });
  ['newSectionTitle','newSectionBody','newSectionImage','newSectionBtnText','newSectionBtnLink'].forEach(id => $(id).value = '');
  renderSectionsList();
});
...
function gatherFormData() {
  const site = {
    name: $('ad-name').value,
    title: $('ad-title').value || $('ad-name').value + ' — HOLEPIC',
    ministry: $('ad-name').value,
    logo: $('ad-logo').value || 'HOLEPIC',
    ...
    x: $('ad-x').value,
    footer: $('ad-footer').value || `© ${new Date().getFullYear()} ${$('ad-name').value || 'HOLEPIC'}. All rights reserved.`,
